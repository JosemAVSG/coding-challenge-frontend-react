...
const Pag=({ resultsPerPage, currentPage,setCurrentPage, totalResults  })=>{
  ...
  const onSpecific =(n)=>{
    ...
    return (
        ...
        <Pagination.Prev className={currentPage === 1 ? "disabled" : ""} onClick={onPrevious}>Prev</Pagination.Prev>
        
        
  
          {pagNumber.map(nopage=>(

            <Pagination.Item key={nopage} 
                       
            onClick={()=>onSpecific(nopage)}
            className={nopage === currentPage ? "active" : ""}
            >{nopage}</Pagination.Item>

          ))}
        
       
        <Pagination.Next className={currentPage >= pagNumber.length ? "disabled" : ""} onClick={onNext}>Next</Pagination.Next>
